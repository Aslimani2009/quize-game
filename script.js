document.addEventListener("DOMContentLoaded", () => {
  ...
  // Corrected and cleaned questions array
  const questions = [
   {
  ...
  function renderGame() {
    ...
    const shuffledAnswers = shuffleArray([...current.answers]); // Shuffle answers
    const correctIndex = shuffledAnswers.indexOf(current.correctAnswer); // Track correct answer index

    questionContainer.innerHTML = `
      <h2>${current.question}</h2>
      <div class="flag ${current.flagClass}"></div>
      <div class="options">
        ${shuffledAnswers.map((answer, index) => `
          <button class="option-btn" onclick="checkAnswer(${index}, ${correctIndex})">${answer}</button>
        `).join("")}
      </div>
      <div class="score-panel">Score: ${score} | Correct: ${correctAnswers} | Attempts: ${attempts}</div>
      <button class="exit-button" onclick="stopGame()">Exit</button>
    `;
  }
  ...
  // Set the background music volume and play it
  bgMusic.volume = 0.3;
  bgMusic.play();
